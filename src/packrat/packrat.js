...
export default async function downloadBlob(packratID, {startsWith = '', endsWith = '', contains = '', doesNotContain = ''} = {}, asString)
{
  ...
  try {
    ...
    if (doesNotContain) {
      regExp = new RegExp(`^(?=^${startsWith})(?=.*${endsWith}$)(?=.*${contains})(?=^(?:(?!${doesNotContain}).)*$).*$`, 'i');
    } else {
      regExp = new RegExp(`^(?=^${startsWith})(?=.*${endsWith}$)(?=.*${contains}).*$`, 'i');
    }
